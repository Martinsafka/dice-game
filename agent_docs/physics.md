...
The Rapier/dice deep dive — the heart of the game. Authoritative for the tray, the die body, **settle detection**, reading the value from the quaternion, cocked-die handling, fairness, the throw, and the drag handoff. For the runtime *structure* (state boundary, component tree, result flow), see `architecture.md`.
...
export function readDieValue(rb: RapierRigidBody) {
  ...
  for (const f of FACES) {
    ...
    const d = _n.dot(UP);
    if (d > best) { best = d; value = f.value; }
  }
...
function SettleWatcher({ dice }: { dice: RefObject<RapierRigidBody>[] }) {
  ...
  useFrame(() => {
    ...
    if (!calm) { stable.current = 0; return; }

    if (++stable.current > 12) {                  // ~0.2 s at 60 fps
...
**Alternative:** Rapier exposes `rb.isSleeping()`. Auto-sleep can help *or* fight the detector (it may sleep a die that's still settling, or sleep late). **Start with velocity thresholding** — you have full control — and consider `isSleeping` only while tuning.
