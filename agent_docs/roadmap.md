...
### M0 — Scaffold
- [ ] Vite + R3F + `@react-three/drei` + `@react-three/rapier` + Zustand project (pnpm, TS, ESLint + Prettier).
- [ ] Orthographic camera looking straight down; basic lights; one **static** box rendered.
- [ ] `pnpm dev` / `build` / `typecheck` / `lint` wired.
